...
% Add path to data
addpath(genpath('...')); % Data from an example participant is included in the github repository

% Add path to other info
addpath(genpath('...\other info'));

% Path to the participants data folder
MainDir = '...';
...
for pp = 2 %Participant AMC002
    ...
    %%
    pp_dir = strcat(MainDir,'\', SubjectName(pp).name, '...');
    cd(fullfile(pp_dir));
    ...
    load('cue_pert.mat') % Data from the github repository
    ...
    [cue, Fs_cue]= audioread('cue_pert.wav'); % Data from the github repository
